...
//  Add software SPI support. Refer: http://www.das-labor.org/wiki/RFM12_library/en
// 2013-06-26 edwin chen<edwin@dragino.com> 
...
#if defined(RFM12_SPI_SOFTWARE)   //use software SPI

#define RFM_IRQ     2
//Pin that the RFM12's slave select is connected to
#define SS_DDR      DDRD  
#define SS_PORT     PORTD
#define SS_BIT      3     // PD3 in MRFM12 module

#define SPI_SS      3    // PD3 in MRFM12 module 
#define SPI_MOSI    5    // PD5 in MRFM12 module,SDI
#define SPI_MISO    6    // PD6 in MRFM12 module,SDO
#define SPI_SCK     4    // PD4 in MRFM12 module
...
void rf12_spiInit () {
    bitSet(SS_PORT, cs_pin);  // Release RFM12B, Set SS_BIT   
    bitSet(SS_DDR, cs_pin);   // set SS_BIT direction to output
...
#if RFM12_SPI_SOFTWARE
	uint8_t x, d = d;
	for (x = 0; x < 8; x++) {   //send and receive a bit each time
		if (out & 0x80) {
			bitSet(PORTD,SPI_MOSI);  // set SPI_MOSI
		} else {
			bitClear(PORTD,SPI_MOSI); // clear SPI_MOSI
		}
		bitSet(PORTD,SPI_SCK);//Set SCK 
		d <<= 1;
		if (PIND & (1<<SPI_MISO)) {    // check SPI_MISO is 0 or 1
			d |= 1;   // get MISO to d
		}
		bitClear(PORTD,SPI_SCK); //Clear SCK
		out <<= 1;
	}
	return d;
...
#if F_CPU > 10000000 && !(RFM12_SPI_SOFTWARE)
    bitSet(SPCR, SPR0);
#endif
...
#if OPTIMIZE_SPI || RFM12_SPI_SOFTWARE 
